...
YEARONEINCOME = 1.0
LAPSEUTILIZATION = "LapseUtilizationLIM15QUnQx.xlsx"
# empty line
# empty line
# empty line
# empty line
...
def morty(key):
    sex = key[0]
    nyears = THISYEAR - 2012 # as this is 2012 mortality table/improvement scale
    age = int(key[1:])
    if sex == 'F': 
        table = MORT_Fq
        improve = 1 - MORT_FG2
    ...
    result = table['q'] * np.power(improve['G2'],table['n']) * MORT_F['F'] * .95 # PJE 20/11/11
    result[120] = 1 # force this to stay 1 so everyone terminates at 120
    return result
...
def punk(inconame, queues, peepframe, years, pmort):
    ...
    for y in years:
        ...
        people['premium'] = premiums[y]
        people['wmfees'] = wmfees[y]
        people['expenses'] = expenses[y]
        people['cashflow'] = cashflow[y]
        #
